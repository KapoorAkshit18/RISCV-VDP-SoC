...
# 36. What Not to Claim During the Presentation

Do **not** claim:

- 100% UVM coverage unless measured.
- UVM regression PASS unless actually run.
- Sensor PASS unless demonstrated end-to-end.
- RF PASS unless demonstrated.
- VDP end-to-end PASS unless demonstrated.
- Specific FPGA frequency unless measured.
- Specific LUT/FF/BRAM utilization unless obtained from Vivado.
- ASIC area/power unless synthesized/analyzed.
- Publication of this project unless actually published.
- Novel CPU architecture.

A technically honest partial result is stronger in a viva than an unsupported claim.

---

# 37. Final Project Summary

RISCV-VDP-SoC establishes a compact RISC-V-based SoC architecture in synthesizable RTL.

The processor generates native memory transactions. A CPU bus adapter isolates processor-specific signaling from the SoC bus. The memory interconnect performs deterministic system address decoding and routes transactions to RAM and memory-mapped peripherals. Peripheral windows use 12-bit local addresses, simplifying register-level implementation.

The current implementation includes RAM, GPIO, sensor status, RF telemetry and VDP/VGA functionality. The directed verification environment has demonstrated correct interconnect routing, local-address conversion, response propagation, byte-strobe handling, unmapped-access behavior and selected-slave operation. GPIO functionality has also been demonstrated.

The remaining verification work is centered on completing end-to-end peripheral validation and building the UVM layer with reusable transactions, constrained-random stimulus, predictors/checkers, assertions and functional coverage.

The project therefore demonstrates an end-to-end engineering workflow:
...
FPGA Validation
     ↓
Future ASIC-Oriented Flow
```

---

# 38. One-Minute Viva Explanation

> “My project is RISCV-VDP-SoC, a RISC-V-based memory-mapped System-on-Chip. I use a PicoRV32-class processor whose native memory interface is connected through a CPU bus adapter to a custom SoC memory interconnect. The interconnect decodes a 32-bit system address and routes transactions to RAM, GPIO, RF telemetry, sensor status and VDP peripherals. Each peripheral has a 4-KB window and therefore receives a 12-bit local address. The VDP additionally operates with an independent pixel clock for VGA generation.
>
> For verification, I first used directed SystemVerilog tests to establish the correctness of the interconnect. These tests cover address decoding, local-address conversion, response propagation, byte strobes, unmapped accesses and slave selection, and the directed interconnect tests pass. GPIO functionality is also demonstrated. Sensor and the remaining peripheral end-to-end validation are the next stage, followed by UVM with constrained-random testing, assertions and functional coverage.
>
> The main engineering contribution is not a new processor, but the integration and verification architecture that combines a compact RISC-V CPU, native-bus adaptation, heterogeneous memory-mapped peripherals and a display clock domain in a reusable SoC structure.”
