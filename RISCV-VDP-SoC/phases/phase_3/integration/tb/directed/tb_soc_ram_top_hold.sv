...
module tb_soc_ram_top;
    ...
    initial begin
    $readmemh("../../phase_1/firmware_test2/firmware_new.hex", dut.ram.mem);
    end
    ...
    always @(posedge clk) begin
    if (dut.m_valid && dut.m_write) begin
        if (dut.m_addr >= 32'h00001220 &&
            dut.m_addr <= 32'h0000129C) begin

            $display("OUTPUT WRITE: time=%0t addr=%h wdata=%h mstrb=%h ready=%b",
                     $time,
                     dut.m_addr,
                     dut.m_wdata,
                     dut.m_strb,
