...
cpu_soc_ram_top #(
    ...
    initial begin
        ...
        if (workload_done) begin
            $display("");
            $display("==============================================");
            $display("PHASE 3 WORKLOAD COMPLETE");
            $display("==============================================");
            $display("Hidden region : 0x00001340 - 0x0000134C");
            $display("Result region : 0x00001350 - 0x0000135C");
            $display("");
            $display("Hidden values:");
            for (i = 0; i < 4; i = i + 1) begin
                $display("hidden[%0d] addr=0x%08h data=0x%08h", i, SIG_BASE + (i * 4), dut.ram.mem[SIG_RAM_INDEX + i]);
            end
            $display("");
            $display("Final values:");
            for (i = 0; i < 4; i = i + 1) begin
                $display("result[%0d] addr=0x%08h data=0x%08h", i, OUTPUT_BASE + (i * 4), dut.ram.mem[OUTPUT_RAM_INDEX + i]);
            end
            $display("");
            $display("Output writes observed = %0d", output_write_count);
            $display("Output reads observed  = %0d", output_read_count);
            $display("No invented expected sigmoid values are compared here.");
            $display("Waiting for 0x33333333...");
            fork : FINAL_WAIT
                begin wait (benchmark_done || verification_failed || dut.trap); end
                begin repeat (10000) @(posedge clk); simulation_timeout = 1'b1; end
            join_any
            disable FINAL_WAIT;
        end
